...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\DELL\\Desktop\\psychopie_final\\word_priming\\word priming_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    slider = visual.Slider(win=win, name='slider',
        ...
        labels=["Familliar","Unfamiliar"], ticks=(1, 2, 3, 4, 5), granularity=0.0,
        style='rating', styleTweaks=(), opacity=None,
        labelColor='LightGray', markerColor='Red', lineColor='White', colorSpace='rgb',
        font='Noto Sans', labelHeight=0.05,
        flip=False, ori=0.0, depth=-1, readOnly=False)
    
    # --- Initialize components for Routine "test_rotine" ---
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "test_rotine" ---
        # create an object to store info about Routine test_rotine
        test_rotine = data.Routine(
            name='test_rotine',
            components=[text_2, textbox, text_3, mouse],
        )
        test_rotine.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        text_2.setText(test)
        textbox.reset()
        ...
        # store start times for test_rotine
        test_rotine.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        test_rotine.tStart = globalClock.getTime(format='float')
        test_rotine.status = STARTED
        thisExp.addData('test_rotine.started', test_rotine.tStart)
        test_rotine.maxDuration = None
        # keep track of which components have finished
        test_rotineComponents = test_rotine.components
        for thisComponent in test_rotine.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "test_rotine" ---
        test_rotine.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                test_rotine.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in test_rotine.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "test_rotine" ---
        for thisComponent in test_rotine.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for test_rotine
        test_rotine.tStop = globalClock.getTime(format='float')
        test_rotine.tStopRefresh = tThisFlipGlobal
        thisExp.addData('test_rotine.stopped', test_rotine.tStop)
        ...
        if test_rotine.maxDurationReached:
            routineTimer.addTime(-test_rotine.maxDuration)
        elif test_rotine.forceEnded:
            routineTimer.reset()
        else:
